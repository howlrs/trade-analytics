...
from scripts.fetch_funding_rate import (
    ...
    fetch_bybit_funding_rates_rest,
    output_path,
    to_dataframe,
    to_ms,
)
...
class TestFetchAndTransform:
    ...
    def bybit_fr(self):
        ...
        data = fetch_bybit_funding_rates_rest("BTC/USDT:USDT", since, end)
        return to_dataframe(data)
    ...
    def test_binance_no_duplicates(self, binance_fr):
        assert binance_fr["timestamp"].n_unique() == len(binance_fr)
    ...
    def test_bybit_no_duplicates(self, bybit_fr):
        assert bybit_fr["timestamp"].n_unique() == len(bybit_fr)
