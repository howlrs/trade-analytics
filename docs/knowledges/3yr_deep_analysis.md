...
## 11. Paper Trader Walk-Forward検証（FR修正後）

### FR単位バグの修正

Drift Data APIの`funding_rate`は**USD/base/hour**（price units）。
正しい変換: `fr_pct = funding_rate / oracle_price_twap`

修正前: Sharpe 25, FR carry 100% → **完全に誤り**
修正後: Sharpe 0.30 (Aggressive), スプレッド捕獲 104%, FR ≈ 0%

### グリッドサーチ最適パラメータ

| gamma | inv_limit | Sharpe | PnL | MaxDD | Calmar |
|-------|-----------|--------|-----|-------|--------|
| 0.12 | 2 | **1.21** | $639 | $270 | 2.37 |
| 0.15 | 2 | 1.20 | $636 | $265 | 2.40 |
| 0.1 | 3 | 0.74 | $549 | $390 | 1.41 |

**inv_limit=2が圧倒的に優位** — 在庫リスク制限がシャープ改善の鍵。

### Walk-Forward検証（3期間分割）

| 期間 | Sharpe | PnL | MaxDD |
|------|--------|-----|-------|
| 2022-2023 (Bear~Recovery) | **-1.90** | -$85 | $96 |
| 2024-mid2024 (Bull) | **+3.47** | +$813 | $205 |
| mid2024-2026 (Mixed) | **-0.64** | -$125 | $162 |

### 結論

**❌ Walk-Forward崩壊: 2024年Bull以外で損失。**

全3年のSharpe 1.21は2024年Bull相場のアーティファクト。
方向性αと同じパターン — レジーム跨ぎでの普遍性なし。

**純粋なAS MM（スプレッド捕獲のみ）はDriftでも持続可能ではない。**
FR carry (13% APR) は存在するが、MM経由で取るほどではない（単純short carry戦略の方が効率的）。

### 次のステップ候補

1. **FR carry × vol timing**: FR carry を vol低い時に集中取得
2. **Drift AS環境のトレンド**: 2026年はAS改善中（-7.3bp）→ 直近データのみの検証
3. **L2 snapshot分析**: 実際のオーダーブック形状でAS回避精度を上げる
4. **他ペアへの展開**: BTC-PERP, ETH-PERP でより安定したレジーム非依存性があるか
